...
<template>
  <div class="space-y-6">
    ...
    <div v-if="data" class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div class="card">
        <div class="section-title mb-3 text-emerald-700">수익 (Revenue)</div>
        <table class="table">
      ...
      <div class="card">
        <div class="section-title mb-3 text-red-700">비용 (Expense)</div>
        <table class="table">
          <thead><tr><th>계정</th><th class="text-right">금액</th></tr></thead>
          <tbody>
      ...
      <div class="card lg:col-span-2">
        <div class="flex items-center justify-between">
          <div>
            <div class="text-sm muted num">{{ data.period_start }} ~ {{ data.period_end }}</div>
            <div class="text-lg font-semibold">당기순이익 (Net Income)</div>
          </div>
          <div class="text-3xl font-bold num" :class="Number(data.net_income) >= 0 ? 'text-brand-600' : 'text-red-600'">
            {{ krw(data.net_income) }}
          </div>
        </div>
      </div>
