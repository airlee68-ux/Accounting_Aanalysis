...
- **[예비 회계사용 온보딩 가이드](docs/onboarding_for_cpa.md)** — 이 시스템의 범위·데이터 모델·리포트 산출 방식·한계를 회계사 관점에서 정리
- [실습용 샘플 엑셀 (12개월치)](samples/transactions_sample_12months.xlsx) — 엑셀 업로드 기능에 바로 쓸 수 있는 가상 거래 데이터
- [작업 로그](docs/) — 날짜별 구현·수정 내역
...
cp .env.example .env
python -m app.seed             # 기본 시드 (간단 샘플)
# 또는 교육용 풍부한 12개월치 데이터:
python -m app.seed_education
...
# 실제 삭제 — 대화식 yes 확인 추가로 필요
python -m app.scripts.reset_transactions --yes
```
계정과목·카테고리 마스터는 유지되고 거래·분개만 삭제됩니다.
